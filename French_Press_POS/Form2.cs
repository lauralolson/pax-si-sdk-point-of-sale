...
namespace French_Press_POS
{
    public partial class Form2 : Form
    {
        ...
        //clicking the Checkout button will perform an auth and immediately capture it.
        //see the “SDK payment function definitions and examples” section of the
        ...
        //clicking the Start Tab button will authorize a transaction without capturing it
        private void startTabButton_Click(object sender, EventArgs e)
        {
            //after a new batch is created, transactionId and authId must have a positive int value, so they must be
            //incremented before the if condition is met and the Auth transaction method is called.
            FormProvider.OrderSummaries.authId = (Int32.Parse(FormProvider.OrderSummaries.transactionId) + 1).ToString();
            ...
            string[] authResult = CommonPayment.transaction(1, "Transaction Authorized", "0", totalValue.Text);
            //if the result of calling the Auth transaction method is not 0,
            //decrement the transaction and auth IDs by 1 so that they are reverted to the original values.
            if (authResult[0] != "0")
            {
                FormProvider.OrderSummaries.transactionId = (Int32.Parse(FormProvider.OrderSummaries.transactionId) - 1).ToString();
                FormProvider.OrderSummaries.authId = (Int32.Parse(FormProvider.OrderSummaries.authId) - 1).ToString();
        ...
        //clicking the Update Tab button will adjust the original auth amount to auth the new order total, if the two amounts are different
        private void updateTabButton_Click(object sender, EventArgs e)
        {
            //depending on the use case, it may not be necessary to perform an incremental auth
            //when new items are added to the tab, but this demo includes the option to do so for educational purposes.
            decimal authAmt = decimal.Parse(authAmtValue.Text.Trim('$'));
            decimal totalAmt = decimal.Parse(totalValue.Text.Trim('$'));
            //after a tab is started and authorized, if additional items are added to the tab the auth amount may be updated
            //by finding the difference between the original auth amount and the new total
            decimal newAuthAmt = totalAmt - authAmt;

            //if the the new total is not equal to the original auth amount, execute the code below
            if (newAuthAmt != authAmt)
            {
                //call the transaction method of the CommonPayment class passing in 41 for the transaction type, which is used to adjust an existing auth.
                string[] adjustResult = CommonPayment.transaction(41, "Amount Adjusted", (string)FormProvider.OrderSummaries.selectedOrder[0]["AuthId"], newAuthAmt.ToString(), "Amount Adjusted");
                //if the result of calling the Auth Adjust transaction method is 0, execute the code below, otherwise do nothing else.
                if (adjustResult[0] == "0")
                {
                    //increment the transactionId by 1
                    transactionIdValue.Text = (string)FormProvider.OrderSummaries.selectedOrder[0]["TransactionId"];
                    authIdValue.Text = (string)FormProvider.OrderSummaries.selectedOrder[0]["AuthId"];
                    //display the updated auth amount on the UI
                    authAmtValue.Text = totalValue.Text;
                    updateOrder();
                    updateDb();
                }
            }
            //if the the new total and the original auth amount are equal, display the message below and don't perform an incremental auth
            else
            {
                MessageBox.Show("The authorized amount hasn't changed.", "Update Tab");
            }
        }

        //clicking the Add Tip button will add the tip amount to the original auth amount
        private void tipButton_Click(object sender, EventArgs e)
        {
            //if the user hasn't entered a value in the tipValue textbox, display the message below
            if (tipValue.Text == "0.00")
            {
                MessageBox.Show("Enter a tip amount in the 'Tip' field on the screen.", "Tip Adjustment");
            }
            else {
                //call the transaction method of the CommonPayment class passing in 41 for the transaction type,
                //which is used to adjust an existing auth.
                //pass in tipValue for the transaction amount.
                //The amount sent in the adjustment request will be added to the existing approved Auth amount.
                //For example, if the original authorized amount was $5.00 and $1.00 is sent in the Auth Adjust request,
                //the new authorized amount will be $6.00.
                string[] adjustResult = CommonPayment.transaction(41, "Amount Adjusted", (string)FormProvider.OrderSummaries.selectedOrder[0]["AuthId"], tipValue.Text, "Amount Adjusted");
                //if the result of calling the Auth Adjust transaction method is 0, execute the code below, otherwise do nothing else.
                if (adjustResult[0] == "0") {
                    transactionIdValue.Text = (string)FormProvider.OrderSummaries.selectedOrder[0]["TransactionId"];
                    authIdValue.Text = (string)FormProvider.OrderSummaries.selectedOrder[0]["AuthId"];
                    updateOrder();
                    //display the new total auth amount on the UI
                    authAmtValue.Text = totalValue.Text;
                    updateDb();
                }
            }
        ...
        //clicking the Pay Tab button will capture the auth
        private void payTabButton_Click(object sender, EventArgs e)
        {
